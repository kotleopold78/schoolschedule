			...
			for ( $i = 0; $i < $cols; $i ++ ) {
				...
				if (isset( $r[ $i ] )){
					...
					if (($lele>0)&&($r[$i]!="")&&($i!=0)){
						...
						//echo $data;
						if ($newws==0){
						$sql="Select * from uroki.peremen where begin=".$data;
						echo $sql;
						$result=mysqli_query($link, $sql);
						...
						if (($obnow==1)&&($dayobn==1)){
							$sql= "delete from uroki.peremen where begin=".$data;
							//echo $sql."<br>";
							//$sql = mysqli_real_escape_string($link, $sql);
							$result=mysqli_query($link, $sql);
							$dayobn=0;
						}
						$sql="INSERT INTO uroki.peremen(`class`, `lesson`, `predmet`, `begin`) VALUES (".$i.",".$lele.",'".$r[$i]."',".$data.")";
